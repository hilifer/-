...
export default function PatientPrescriptionDetailPage({
  ...
  // Fallback: legacy tongueImage / faceImage from Diagnosis (base64 or URL)
  ...
  // Parse original AI recommended herbs from diagnosis
  const originalHerbs: HerbEntry[] = diagnosis?.formulaHerbs
    ? JSON.parse(diagnosis.formulaHerbs)
    : [];

  const adoptionInfo = prescription?.adoptionLevel
    ? ADOPTION_LABELS[prescription.adoptionLevel]
    : null;
